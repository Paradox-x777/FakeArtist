...
namespace FreeDraw
{
    ...
    public class Drawable : MonoBehaviourPunCallbacks
    {
        [Header("Components")]
        ...
        bool mouse_was_previously_held_down_in_frame = false;
        bool no_drawing_on_current_drag = false;

        public delegate void DrawingEvent();
        public static event DrawingEvent OnPenReleased;
        ...
        {
            var colorHEX = ColorUtility.ToHtmlStringRGBA(Pen_Colour);
            photonView.RPC("DefaultPenBrush", RpcTarget.All, world_point, $"#{colorHEX}");
        }

        [PunRPC]
        public void DefaultPenBrush(Vector2 world_point, string color)
        {
            Vector2 pixel_pos = WorldToPixelCoordinates(world_point);
            Color drawingColor = new Color();

            if (ColorUtility.TryParseHtmlString(color, out drawingColor)) {
                cur_colors = drawable_texture.GetPixels32();
        ...
        [PunRPC]
        public void PenReleased()
        {
            previous_drag_position = Vector2.zero;
            no_drawing_on_current_drag = false;
            if (mouse_was_previously_held_down_in_frame && OnPenReleased != null)
            {
                OnPenReleased();

            }
            mouse_was_previously_held_down_in_frame = false;
        }
            ...
            if (IsEnabled)
            {
                ...
                if (mouse_held_down && !no_drawing_on_current_drag)
                {
                    ...
                    if (hit != null && hit.transform != null)
                    {
                        ...
                        mouse_was_previously_held_down_in_frame = true;                        
                ...
                else if (!mouse_held_down)
                {
                    ...
                    //}
                    //mouse_was_previously_held_down_in_frame = false;
                    photonView.RPC("PenReleased", RpcTarget.All);
                }
